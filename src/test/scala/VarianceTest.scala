...
// 1) use contravariant and covariant type checks at compile-time
...
// then we should make its methods:
//   update() method contravariant, so that the subclass can perform the update() method where the superclass does
//   get() method covariant       , so that the subclass can perform the update() method where the superclass does
// ex.
...
// however, it can lead to contradiction if the Array is mutable
//   val e1: NonEmpty = arr1.get()    // the element is updated to Empty, which is NOT NonEmpty

// the contravariant and covariant checks make sure that the up-casting is safe, and help identify the contradiction at compile time
// i.e. MutableArray[IntSet, NonEmpty] <: MutableArray[IntSet, IntSet]
...
class MutableArrayImpl[T, R](val e: R) extends MutableArray[T, R] {
  override def update(x: T): MutableArray[T, R] = {
    ...
    this               //   the variance check helps to identify the above error at compile-time
  }
  override def get(): R = this.e
}

// 2) use type bound reference at compile-time
